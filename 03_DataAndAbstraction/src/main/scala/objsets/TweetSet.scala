...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("No such element in empty list")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    // may be done in the way explained during lecture:
    // ((left union right) union that) incl elem

    // or may be done in the context of filterAcc assuming that p(Tweet) will always return true
